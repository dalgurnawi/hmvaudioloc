...
def create_paper_code_7_model(dropout_rate=0.5):
    spec_start = Input(shape=(64, 64, 2))
    spec_cnn = spec_start
    spec_cnn = tf.keras.layers.Conv2D(filters=32, kernel_size=(1, 16), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.Activation('relu')(spec_cnn)
    spec_cnn = tf.keras.layers.BatchNormalization()(spec_cnn)

    spec_cnn = tf.keras.layers.Conv2D(filters=32, kernel_size=(1, 8), padding="same")(spec_cnn)
