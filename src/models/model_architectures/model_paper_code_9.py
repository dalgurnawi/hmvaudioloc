...
def create_paper_code_9_model(dropout_rate=0.5):
    spec_start = Input(shape=(64, 64, 2))
    spec_cnn = spec_start
    spec_cnn = tf.keras.layers.Conv2D(filters=32, kernel_size=(3, 32), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.Activation('relu')(spec_cnn)
    spec_cnn = tf.keras.layers.BatchNormalization()(spec_cnn)

    spec_cnn = tf.keras.layers.Conv2D(filters=32, kernel_size=(2, 16), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.MaxPooling2D(pool_size=(1, 4), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.Activation('relu')(spec_cnn)
    spec_cnn = tf.keras.layers.BatchNormalization()(spec_cnn)

    spec_cnn = tf.keras.layers.Conv2D(filters=64, kernel_size=(2, 32), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.Activation('relu')(spec_cnn)
    spec_cnn = tf.keras.layers.BatchNormalization()(spec_cnn)

    spec_cnn = tf.keras.layers.Conv2D(filters=64, kernel_size=(3, 4), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.MaxPooling2D(pool_size=(1, 4), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.Activation('relu')(spec_cnn)
    spec_cnn = tf.keras.layers.BatchNormalization()(spec_cnn)
    ...
    spec_cnn = tf.keras.layers.Conv2D(filters=512, kernel_size=(2, 3), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.Activation("relu")(spec_cnn)
    spec_cnn = tf.keras.layers.BatchNormalization()(spec_cnn)

    spec_cnn = tf.keras.layers.Conv2D(filters=512, kernel_size=(3, 4), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.MaxPooling2D(pool_size=(1, 2), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.Activation("relu")(spec_cnn)
    spec_cnn = tf.keras.layers.BatchNormalization()(spec_cnn)

    spec_cnn = tf.keras.layers.Conv2D(filters=512, kernel_size=(1, 3), padding="same")(spec_cnn)
