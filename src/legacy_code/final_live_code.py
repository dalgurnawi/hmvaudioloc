...
def recorder(start, mic_num):
    ...
    q = Queue()
    file = sf.SoundFile(f'./recordings_multimic/temp_{mic_num}_file_{file_num}.wav',
                        mode='w',
                        samplerate=RATE,
                        channels=CHANNELS)
...
def map_sample_files(mic_nums, file_check_num):
    # file_num = next(my_iterator)
    try:
        audio_tensor_1 = tfio.audio.AudioIOTensor(f"./recordings_multimic/temp_{mic_nums[0]}_file_{file_check_num}.wav").to_tensor()
        audio_tensor_2 = tfio.audio.AudioIOTensor(f"./recordings_multimic/temp_{mic_nums[1]}_file_{file_check_num}.wav").to_tensor()
...
def model_querry(direction_model, elevation_model, microphone_numbers, file_check_num):
    test_input = map_sample_files(microphone_numbers, file_check_num)
    # if len(test_input) == 0:
    #     return None
...
if __name__ == '__main__':
    ...
    p1 = Process(target=recorder, args=(start, mic_nums[0]))
    p2 = Process(target=recorder, args=(start, mic_nums[1]))
    ...
    while True:
        ...
        try:
            time.sleep(0.1)
            try:
                (sector_output_probs, sector_output_labels, elevation_output_probs, elevation_output_labels) = model_querry(reconstructed_direction_model, reconstructed_elevation_model, mic_nums, file_check_num)
            except ValueError:
