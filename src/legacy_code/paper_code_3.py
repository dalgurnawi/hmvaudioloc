...
def get_seldtcn_model(dropout_rate):
    ...
    spec_cnn = spec_start
    spec_cnn = tf.keras.layers.Conv2D(filters=32, kernel_size=(1, 4), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.Activation('relu')(spec_cnn)
    spec_cnn = tf.keras.layers.BatchNormalization()(spec_cnn)
    
    spec_cnn = tf.keras.layers.Conv2D(filters=32, kernel_size=(1,32), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.MaxPooling2D(pool_size=(1,8), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.Activation('relu')(spec_cnn)
    spec_cnn = tf.keras.layers.BatchNormalization()(spec_cnn)
    
    spec_cnn = tf.keras.layers.Conv2D(filters=64, kernel_size=(3,32), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.Activation('relu')(spec_cnn)
    spec_cnn = tf.keras.layers.BatchNormalization()(spec_cnn)
    ...
    spec_cnn = tf.keras.layers.Conv2D(filters=64, kernel_size=(1,2), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.Activation("relu")(spec_cnn)
    spec_cnn = tf.keras.layers.BatchNormalization()(spec_cnn)
    
    spec_cnn = tf.keras.layers.Conv2D(filters=64, kernel_size=(2,2), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.MaxPooling2D(pool_size=(2,4), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.Activation("relu")(spec_cnn)
    spec_cnn = tf.keras.layers.BatchNormalization()(spec_cnn)
    
    spec_cnn = tf.keras.layers.Conv2D(filters=128, kernel_size=(2,4), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.Activation("relu")(spec_cnn)
    spec_cnn = tf.keras.layers.BatchNormalization()(spec_cnn)
    
    spec_cnn = tf.keras.layers.Conv2D(filters=128, kernel_size=(1,8), padding="same")(spec_cnn)
    spec_cnn = tf.keras.layers.Activation("relu")(spec_cnn)
    spec_cnn = tf.keras.layers.BatchNormalization()(spec_cnn)
...
plt.legend()
plt.savefig("history1_paper_code_3.png")
plt.close()

plt.plot(history.history["accuracy"], label="accuracy")
plt.plot(history.history["val_accuracy"], label="val_accuracy")
plt.legend()
plt.savefig("history2_paper_code_3.png")
plt.close()

model.save('paper_code_3.model')