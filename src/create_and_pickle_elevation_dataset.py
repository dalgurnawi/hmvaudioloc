...
def map_dataset():
    ...
    for file_name in tqdm(list_of_all_files):
        ...
        label_string_from_filename = file_name.split("_")[-3]
        if label_string_from_filename == "45down":
            label = tf.convert_to_tensor(int(0))
        if label_string_from_filename == "level":
            label = tf.convert_to_tensor(int(1))
        if label_string_from_filename == "45up":
            label = tf.convert_to_tensor(int(2))
...
tf.data.experimental.save(dataset, "pickled_elevation_dataset.dat")
